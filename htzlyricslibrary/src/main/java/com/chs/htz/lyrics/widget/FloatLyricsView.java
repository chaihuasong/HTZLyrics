...
public class FloatLyricsView extends AbstractLrcView {
    ...
    private void drawDynamicLyrics(Canvas canvas) {
        ...
        //android.util.Log.d("huasong", "curLyrics:" + curLyrics + " splitLyricsRealLineNum:" + splitLyricsRealLineNum + " size:" + splitLyricsLineInfos.size() + " splitLyricsLineNum:" + splitLyricsLineNum);
        if (splitLyricsLineNum > 0) {
    ...
    public void setLyricsReader(LyricsReader lyricsReader) {
        ...
        } else if (lyricsReader != null && lyricsReader.getLyricsType() == LyricsInfo.LRC) {
            int extraLrcType = getExtraLrcType();
            //翻译歌词以动感歌词形式显示
            if (extraLrcType == AbstractLrcView.EXTRALRCTYPE_NOLRC) {
                setTranslateDrawType(AbstractLrcView.TRANSLATE_DRAW_TYPE_LRC);
            }
        } else {
